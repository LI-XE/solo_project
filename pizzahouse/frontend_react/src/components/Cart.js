...
const Cart = () => {
    const [{ pagereload, cartuncomplit, delete_cartuncomplit }, dispatch] = useGlobalState()
                ...
                <div className="flex items-center">
                    {
                                ...
                                { 
                                    cartuncomplit?.cartproduct.map((data, i) => (
                                        <tr key= {i} className="row">
                                            ...
                                            <td className="col-3 my-3">{data.product[0].title}</td>
                                            ...
                                            <td className="col-3 my-3">
                                                <button className="btn btn-warning px-3" onClick={()=>editcart(data.id)}>-</button>
                                                <button className="btn btn-primary mx-3 px-3" onClick={()=>updatecart(data.id)}>+</button>
                                                <a className="btn btn-danger" href="#" role="button" onClick={()=>deletecartproduct(data.id)}><img src="img\trash.svg" /></a>
                                            </td>
                                        </tr>
                                    ))
                                }
                        ...
                        <div>                           
                            <div className="container mx-auto text-center">
                                ...
                                <Link to="/layout" className="px-5 py-2 rounded-pill btn-primary text-white font-bold m-5">Go back to order!</Link>
                            </div>
                        </div>
                    }
                    ...
                    <div className="row mt-5">
                        <div className="col">
                            <Link to="/oldorders" className="btn btn-info">Old orders</Link>
                        </div>
                        {
                            cart_product_length != 0 &&
                            <div className="col">
                                <Link to="/layout" className="px-5 py-2 rounded-pill btn-primary text-white font-bold m-5">Order More</Link>
                                <Link className="btn rounded-pill btn-danger px-5 py-1" onClick={()=>deletecart(cartuncomplit?.id)}>Delete Cart</Link>
                            </div>
                        }
                    </div>
                </div>
